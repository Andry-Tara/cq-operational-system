...
begin
  ...
  /*
   * Existing report sections with a complete applicability
   * snapshot are SAFE:
   *
   *   - they are pinned to their exact form version;
   *   - session runtime reads historical snapshot only;
   *   - current outlet facility configuration is not used.
   *
   * Block only when an unfinished report still contains a
   * relevant version section that:
   *
   *   A. has not created its report_section yet, or
   *   B. has an incomplete / invalid facility snapshot.
   */
  select
    r.report_number,
    r.status
    ...
    and exists (
      ...
      where fvs.form_version_id =
        ...
        and (
           ...
           *
           * If facility changes now, future section creation
           * could snapshot the new value into an old report.
           */
          not exists (
            select 1
          ...
          /*
           * Section exists, but at least one exact facility
           * question is missing its historical snapshot.
           */
          exists (
            select 1
            from public.questions q2

            where q2.version_section_id =
                  fvs.id
              ...
              and not exists (
                select 1
                from public.report_sections rs2

                join public.report_question_applicability rqa
                  on rqa.report_section_id =
                     rs2.id
                 and rqa.question_id =
                     q2.id
  ...
  if v_open_report_number
     is not null then
    raise exception
      'OUTLET_FACILITY_OPEN_REPORT: report % is currently % and still has an unsnapshotted facility section for %.',
      v_open_report_number,
      v_open_report_status,
      v_facility_code
      using errcode = '55000';
