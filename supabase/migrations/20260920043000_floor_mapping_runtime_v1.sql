...
-- ============================================================
-- FLOOR MAPPING V1-C
-- DAILY FOH STAFF PINS + ATOMIC DRAFT/SUBMIT
-- ============================================================


create table public.floor_mapping_staff_pins (
  id uuid
    primary key
  ...
  constraint floor_mapping_staff_pin_position_chk
    check (
      btrim(position_label) <> ''
    ),

  constraint floor_mapping_staff_pin_names_chk
    check (
      cardinality(
        assigned_names
      ) > 0
...
create index
floor_mapping_staff_pins_session_sort_idx
on public.floor_mapping_staff_pins (
  session_id,
  sort_order
);


create trigger floor_mapping_staff_pins_set_updated_at
before update
on public.floor_mapping_staff_pins
for each row
...
alter table public.floor_mapping_staff_pins
