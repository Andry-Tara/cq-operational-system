...
function statusLabel(status: AuditorOutletCard["status"]) {
  if (status === "draft") return "IN PROGRESS";
  if (status === "submitted") return "SUBMITTED";
  return "NOT STARTED";
}

function statusClass(status: AuditorOutletCard["status"]) {
  if (status === "draft") {
...
export function AuditorDashboard({
  ...
  const metrics = useMemo(() => {
    return {
      ...
      completed: outlets.filter((row) => row.status === "submitted").length,
      inProgress: outlets.filter((row) => row.status === "draft").length,
      notStarted: outlets.filter((row) => row.status === "not_started").length,
      findings: outlets.reduce(
        (total, row) => total + row.findingCount,
        0,
      ),
    };
  }, [outlets]);

  async function activateOutlet(outletId: string) {
    const response = await fetch("/api/outlet/select", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(
        payload?.error || "Unable to select outlet.",
      );
    }
  }

  async function openAudit(row: AuditorOutletCard) {
    if (busyKey) return;
    ...
    try {
      ...
      await activateOutlet(row.id);

      // Start or resume the draft immediately.
      // /api/audit/session already guarantees one active draft
      // per auditor + outlet and resumes an existing draft.
      ...
      if (!response.ok) {
        throw new Error(
          payload?.error || "Unable to open audit.",
        );
      }

      // Active outlet lives in an HTTP-only cookie.
      // Hard navigation ensures Server Components see it.
      window.location.assign("/protected/audit");
    } catch (exception: any) {
      setError(
        exception?.message || "Unable to open audit.",
      );
      setBusyKey(null);
    }
  }
  ...
  async function openResult(
    ...
    try {
      ...
      setError(
        exception?.message || "Unable to open audit result.",
      );
      setBusyKey(null);
    }
  }
  ...
  return (
    <main className="min-h-screen bg-[#f5f5f3] text-neutral-900">
      <div className="mx-auto max-w-[1480px] px-4 py-6 sm:px-6 md:py-10 lg:px-8">
        ...
        <section className="mt-5 overflow-hidden rounded-[24px] border border-neutral-200 bg-white shadow-sm">
          ...
          <div className="divide-y divide-neutral-100">
            {outlets.map((row) => {
              const loading =
                busyKey === `audit:${row.id}` ||
                busyKey === `result:${row.id}`;
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    ...
                    row.sessionId ? (
                      ...
                      <button
                        ...
                        disabled={busyKey !== null}
                        onClick={() => openAudit(row)}
                        className="inline-flex h-11 min-w-[135px] items-center justify-center rounded-xl bg-red-700 px-5 text-sm font-bold text-white transition hover:bg-red-800 disabled:cursor-wait disabled:opacity-60"
                      >
                        {loading
                          ? "Opening..."
                          : row.status === "draft"
        ...
        <section className="mt-5 overflow-hidden rounded-[24px] border border-neutral-200 bg-white shadow-sm">
          ...
          {!recentAudits.length ? (
            <div className="px-6 py-12 text-center text-sm text-neutral-400">
              No submitted audit history yet.
            </div>
          ) : (
            <div className="divide-y divide-neutral-100">
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-xs text-neutral-500">
                      {audit.findingCount} finding
                      {audit.findingCount === 1 ? "" : "s"}
                      {audit.submittedAt
                        ? ` · ${timeLabel(audit.submittedAt)}`
                        : ""}
                    </p>
                  </div>
