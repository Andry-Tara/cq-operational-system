...
async function getActiveOutletUncached() {
  ...
  const activeOutletId = cookieStore.get("cq_active_outlet")?.value;

  // ==========================================================
  // User has all outlet access?
  // ==========================================================

  const { data: hasAllAccess } = await supabase.rpc("has_all_outlet_access");
  ...
  if (hasAllAccess !== true) {
    ...
    const outlets = (assignments ?? [])
      ...
      .filter((outlet: any) => outlet && outlet.is_active === true);
...
export const getActiveOutlet = cache(getActiveOutletUncached);
