...
async function loadWarehouseContext({
  ...
  if (
    ...
    return {
      error:
        "Warehouse Finalization hanya tersedia untuk Closing Central Kitchen.",
      status: 409,
    } as const;
  }
  ...
  // ==========================================================
  // EXPLICIT WAREHOUSE LEADER AUTHORIZATION
  //
  // IMPORTANT:
  ...
  if (!leaderAssignment) {
    return {
      error:
        "Anda bukan Warehouse Leader untuk report ini.",
      code:
        "NOT_WAREHOUSE_LEADER",
      status: 403,
    } as const;
  }
  ...
  // ==========================================================
  // REQUIRED WAREHOUSE SECTIONS — DYNAMIC
  //
  // Business term : Warehouse
  // Internal DB   : STORE
  //
  // Source of truth:
  // - report.form_id
  // - report.form_version_id
  // - sections.area_code = STORE
  // - form_version_sections.is_active = true
  // - form_version_sections.is_required = true
  //
  // No Warehouse section names/counts are hardcoded.
  // ==========================================================

  const {
    data: warehouseSections,
    error:
      warehouseSectionsError,
  } =
    await admin
      .from("sections")
      .select(`
  ...
  if (
    ...
    return {
      error:
        "Warehouse section definition belum tersedia untuk form report ini.",
      code:
        "WAREHOUSE_SECTION_CONFIG_ERROR",
      status: 409,
    } as const;
  }

  const warehouseSectionIds =
    sectionRows.map(
      (item: any) =>
        item.id
    );
  ...
  // ==========================================================
  // ACTIVE REQUIRED WAREHOUSE VERSION SECTIONS
  // ==========================================================
  ...
  if (
    !activeVersionSections.length
  ) {
    return {
      error:
        "Tidak ada required Warehouse section aktif pada form version report ini.",
      code:
        "WAREHOUSE_VERSION_CONFIG_ERROR",
      status: 409,
    } as const;
  }
  ...
  // evidence that must be used by the Final Warehouse PDF.
  //
...
// Safe to call before all required Warehouse sections are reviewed.
// ============================================================
...
export async function GET(
  ...
  try {
    ...
    console.error(
      "Warehouse finalization readiness error:",
      error
    );
    ...
    return NextResponse.json(
      {
        error:
          error?.message ||
          "Unable to check Warehouse finalization.",
      },
      {
        status: 500,
      }
...
// POST
//
// Commits Warehouse finalization AFTER the PDF has already
  ...
  try {
    ...
    if (!pdfStoragePath) {
      return NextResponse.json(
        {
          error:
            "pdfStoragePath wajib diisi sebelum Finalize Warehouse.",
        },
        {
          status: 400,
        }
      ...
      return NextResponse.json(
        {
          ...
          error:
            `Warehouse belum siap difinalisasi. Submitted ${contextData.submittedCount}/${contextData.requiredCount}, Reviewed ${contextData.reviewedCount}/${contextData.requiredCount}.`,
        },
        {
          status: 409,
        }
    ...
    const expectedPrefix =
      `reports/${contextData.report.id}/area/warehouse/`;
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Warehouse PDF storage path tidak valid.",
          code:
            "INVALID_WAREHOUSE_PDF_PATH",
        },
        {
          status: 400,
        }
    ...
    if (storedPdfError) {
      throw new Error(
        `Unable to verify Warehouse PDF: ${storedPdfError.message}`
      );
    }
    ...
    if (!storedPdf) {
      return NextResponse.json(
        {
          error:
            "Warehouse PDF belum ditemukan di storage. Finalization dibatalkan.",
          code:
            "WAREHOUSE_PDF_NOT_FOUND",
        },
        {
          status: 409,
        }
    ...
    if (
      ...
      if (
        ...
        return NextResponse.json(
          {
            error:
              "Warehouse belum pernah difinalisasi. Gunakan Finalize Warehouse terlebih dahulu.",
            code:
              "WAREHOUSE_NOT_FINALIZED",
          },
          {
            status: 409,
          }
    ...
    // STORE + WAREHOUSE have explicit area finalization.
    // ========================================================
