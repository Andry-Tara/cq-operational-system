...
export default async function BriefingPage() {
  ...
  if (!profile || !outletRow || !assignmentResult.data) {
    redirect("/protected");
  }
  ...
  if (String(outletRow.code).trim().toUpperCase() === "CNT") {
    redirect("/protected");
  }
  ...
  return (
    <BriefingClient
      ...
      businessDate={date}
      outletTimezone={outletRow.timezone || "Asia/Jakarta"}
      picName={profile.full_name || user.email || "Outlet PIC"}
      submittedSessions={todaySessions ?? []}
    />
  );
}
